...
class smart_nav_service():
    ...
    def __init__(self, zeta: float = 0.5, d_goal_thr: float = 2.0, nu: float = 1.0, d_obst_thr: float = 2.0):
        ...
        self.__cur_pos = np.array([0, 0, 0])

        rospy.loginfo(f"Initialized smart_nav service, zeta = {zeta}, nu = {nu}, d_goal_thr = {d_goal_thr}, d_obst_thr = {d_obst_thr}")
    ...
    def update(self):
        ...
        # Send velocity
        self.__set_velocity(vx=vel[0], vy=vel[1], vz=vel[2], frame_id='map', auto_arm=self.__nav_task.auto_arm)
    ...
    def __handle_navigate_global(self, req: clover_srv.NavigateGlobalRequest):
        ...
        return clover_srv.NavigateGlobalResponse(False, 'NOT_SUPPORTED')
...
if __name__ == "__main__":
    ...
    nav_srv: smart_nav_service = smart_nav_service(zeta, d_goal_thr, nu, d_obst_thr)
